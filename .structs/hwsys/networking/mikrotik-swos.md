---
template_id: YS-NET-MIKROTIK-SWOS-001
template_family: hwsys
title: MikroTik SwOS Agent Standard
version: 0.1.0
status: draft
...
Define agent controls for SwOS switch management, backup, VLAN, LAG, and port-state practices.
...
```text
# SwOS is primarily web-managed on supported devices
Record firmware version from UI
Export/backup configuration from UI
Validate VLAN/LAG/port state from UI
```
